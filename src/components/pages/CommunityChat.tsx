...
const initialMessages = {
  ...
  "foundational": [
    { id: 1, user: "Admin", avatar: "bg-gray-200 text-gray-700", time: "Yesterday", text: "Here is a great reading list for beginners in integration work.", reactions: ["📚"], replies: 0, role: "Admin", location: "Global", interests: [], thread: [] },
  ],
  "default": [
    { id: 0, user: "System", avatar: "bg-gray-100 text-gray-600", time: "Now", text: "This is the start of the channel. Be kind and respectful.", reactions: [], replies: 0, role: "System", location: "", interests: [], thread: [] }
  ]
};
...
export function CommunityChat() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] md:h-full w-full bg-white text-[#101010] relative overflow-hidden">
      {/* Sidebar Channel List */}
      <div className={`
        w-full md:w-[280px] flex-shrink-0 bg-[#F9F6F3] border-r border-[#101010]/5 flex flex-col
        ${mobileView === "chat" ? "hidden md:flex" : "flex"}
      `}>
        <div className="h-16 px-6 border-b border-[#101010]/5 flex items-center justify-between">
          <h2 className="font-bold text-[#101010] text-lg tracking-tight flex items-center gap-2">
            <Users className="w-5 h-5 text-[#066237]" />
            Community
          </h2>
        </div>
        
        <div className="flex-1 overflow-y-auto px-4 py-6 space-y-8">
          {channels.map((section, idx) => (
            <div key={idx}>
              <h3 className="px-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-3 flex items-center justify-between group">
                {section.category}
                <Plus className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 cursor-pointer text-[#066237]" />
              </h3>
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${activeChannelId === channel.id 
                        ? "bg-white text-[#066237] font-semibold shadow-sm shadow-gray-200/50" 
                        : "text-muted-foreground hover:bg-[#101010]/5 hover:text-[#101010]"
                      }
                    `}
                  >
                    <HashIcon className={`w-4 h-4 ${activeChannelId === channel.id ? "opacity-100" : "opacity-40 group-hover:opacity-100"}`} />
                    <span className="truncate">{channel.name}</span>
                    {channel.id === 'general' && (
                       <span className="ml-auto w-1.5 h-1.5 rounded-full bg-[#066237]" />
                    )}
                  </button>
                ))}
              </div>
      ...
      `}>
        <div className="h-16 border-b border-[#101010]/5 flex items-center justify-between px-6 shrink-0 bg-white/80 backdrop-blur-md z-10 sticky top-0">
          <div className="flex items-center gap-3 overflow-hidden">
            <button onClick={handleBack} className="md:hidden -ml-2 p-2 text-muted-foreground hover:text-[#101010]">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <div className="min-w-0">
              <h3 className="font-bold text-lg flex items-center gap-2 truncate text-[#101010]">
                <Hash className="w-5 h-5 text-muted-foreground/50 shrink-0" />
                {activeChannel.name}
              </h3>
              <p className="text-xs text-muted-foreground truncate hidden md:block font-medium">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          {currentMessages.map(( msg: ChatMessage) => (
            <div key={msg.id} className={`flex gap-4 group hover:bg-gray-50 -mx-6 px-6 py-3 rounded-xl transition-colors relative ${activeThread?.id === msg.id ? 'bg-[#066237]/5' : ''}`}>
              <div 
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex items-center gap-2">
                  <span 
                    onClick={() => setSelectedUser(msg)}
                    className="font-bold text-[#101010] text-[15px] hover:underline cursor-pointer"
                  >
                    {msg.user}
                  </span>
                  <span className="text-[11px] text-muted-foreground font-medium">{msg.time}</span>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200 ml-2">
                    <button 
                       ...
                       className="text-xs font-medium text-muted-foreground hover:text-[#066237] hover:bg-[#066237]/5 p-1 rounded transition-all"
                     ...
                     <div className="relative">
                       <button 
                          ...
                          className="text-muted-foreground hover:text-yellow-600 p-1 rounded hover:bg-yellow-50 transition-colors"
                          title="Add reaction"
                       >
                       ...
                       {showEmojiPicker === msg.id && (
                         <div className="absolute left-0 top-6 z-20 bg-white shadow-xl border border-gray-100 rounded-xl p-2 flex gap-1 animate-in zoom-in-95 duration-200 w-max">
                           {emojis.map(emoji => (
                             <button
                               key={emoji}
                               onClick={() => handleReaction(msg.id, emoji)}
                               className="hover:bg-gray-50 p-1.5 rounded-lg text-xl transition-colors w-9 h-9 flex items-center justify-center transform hover:scale-110"
                             >
                               {emoji}
                             </button>
                           ))}
                ...
                <p className="text-[#101010] text-[15px] leading-relaxed whitespace-pre-wrap">
                  {msg.text}
                </p>
                ...
                {msg.reactions && msg.reactions.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {msg.reactions.map((emoji: string, i: number) => (
                      <span key={i} className="inline-flex items-center justify-center px-2 py-0.5 bg-gray-50 rounded-lg text-sm border border-[#101010]/5 shadow-sm">
                        {emoji}
                      </span>
                    ))}
                  </div>
                ...
                {(msg.replies > 0) && (
                   <div 
                     onClick={() => setActiveThread(msg)}
                     className="flex items-center gap-3 mt-3 cursor-pointer group/thread w-fit p-1.5 pr-3 -ml-1.5 rounded-lg hover:bg-white border border-transparent hover:border-[#101010]/5 hover:shadow-sm transition-all"
                   >
                     <div className="flex -space-x-2 pl-1">
                        {/* Mock avatars */}
                        <div className="w-5 h-5 rounded-full bg-indigo-100 border-2 border-white"></div>
                        <div className="w-5 h-5 rounded-full bg-emerald-100 border-2 border-white"></div>
                     </div>
                     <span className="text-xs font-bold text-[#066237] group-hover/thread:underline">
                       {msg.replies} replies
                     </span>
                     <span className="text-[10px] text-muted-foreground group-hover/thread:text-[#101010]">
                        Last reply today
                     </span>
                   </div>
                )}
        ...
        <div className="p-6 bg-white relative z-20">
          <div className="border border-[#101010]/10 rounded-2xl shadow-sm bg-white overflow-hidden focus-within:ring-2 focus-within:ring-[#066237]/10 focus-within:border-[#066237] transition-all group">
            <div className="flex items-end gap-3 p-3">
              <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-[#066237] hover:bg-[#066237]/5 rounded-xl">
              ...
              <Button 
                ...
                size="icon" 
                className="h-9 w-9 rounded-xl bg-[#066237] hover:bg-[#05502d] flex-shrink-0 transition-all disabled:opacity-50 disabled:bg-gray-100 disabled:text-gray-400 shadow-sm"
              >
      ...
      {/* THREAD SIDE PANEL */}
      <div className={`
        absolute top-0 right-0 h-full w-full md:w-[380px] bg-white border-l border-[#101010]/10 shadow-2xl
        transform transition-transform duration-300 z-30 flex flex-col
        ${activeThread ? "translate-x-0" : "translate-x-full"}
      `}>
         {/* Thread Header */}
         <div className="h-16 border-b border-[#101010]/5 flex items-center justify-between px-5 bg-[#F9F6F3]/50 backdrop-blur-md">
            <h3 className="font-bold flex items-center gap-2 text-[15px]">
              Thread
              <span className="text-muted-foreground font-normal text-sm">#{activeChannel.name}</span>
            </h3>
            <div className="flex items-center gap-1">
              <Button 
                variant="ghost" 
                size="icon" 
                className="h-8 w-8 rounded-full hover:bg-[#066237]/10 hover:text-[#066237] text-muted-foreground transition-colors"
                onClick={() => toast.success("Thread saved to bookmarks")}
                title="Bookmark thread"
              >
                <Bookmark className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-black/5" onClick={() => setActiveThread(null)}>
                <X className="w-4 h-4" />
              </Button>
            </div>
         </div>
         ...
         <div className="flex-1 overflow-y-auto p-5 space-y-6">
            {activeThread && (
              <>
                {/* Original Message */}
                <div className="flex gap-3 pb-6 border-b border-[#101010]/5">
                   ...
                   <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-baseline gap-2">
                        <span className="font-bold text-[#101010] text-[15px]">{activeThread.user}</span>
                        <span className="text-[11px] text-muted-foreground font-medium">{activeThread.time}</span>
                      </div>
                      <p className="text-[#101010] text-[15px] leading-relaxed whitespace-pre-wrap">
                        {activeThread.text}
                      </p>
                ...
                <div className="relative py-2">
                  <div className="absolute inset-0 flex items-center">
                    <span className="w-full border-t border-[#101010]/5" />
                  </div>
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white px-3 text-muted-foreground font-bold tracking-wider text-[10px]">
                ...
                <div className="space-y-5">
                  {(activeThread.thread || []).map((reply: ChatReply) => (
                    <div key={reply.id} className="flex gap-3 group">
                       <div className={`w-8 h-8 rounded-lg ${reply.avatar || 'bg-gray-200 text-gray-700'} flex-shrink-0 flex items-center justify-center text-[10px] font-bold mt-1 shadow-sm`}>
                         {reply.user.split(' ').map((n:string) => n[0]).join('').slice(0,2)}
                       </div>
                       <div className="flex-1 min-w-0 space-y-0.5">
                          <div className="flex items-baseline gap-2">
                            <span className="font-bold text-[#101010] text-sm">{reply.user}</span>
                            <span className="text-[10px] text-muted-foreground">{reply.time}</span>
                          </div>
                          <p className="text-[#101010] text-sm leading-relaxed whitespace-pre-wrap">
                            {reply.text}
                          </p>
                       </div>
                    </div>
         ...
         {/* Thread Input */}
         <div className="p-5 bg-white border-t border-[#101010]/5">
           <div className="border border-[#101010]/15 rounded-xl shadow-sm bg-white overflow-hidden focus-within:ring-2 focus-within:ring-[#066237]/10 focus-within:border-[#066237] transition-all">
              <div className="flex items-end gap-2 p-2">
                ...
                <Button 
                  ...
                  size="icon" 
                  className="h-8 w-8 rounded-lg bg-[#066237] hover:bg-[#05502d] flex-shrink-0 transition-all disabled:opacity-50 disabled:bg-gray-100 disabled:text-gray-400"
                >
      ...
      {selectedUser && (
        <div className="absolute inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedUser(null)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-[#101010]/5" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-8 pb-10 text-center">
              <div className={`w-28 h-28 rounded-full ${selectedUser.avatar} border-8 border-white shadow-lg mx-auto mb-5 flex items-center justify-center text-3xl font-bold`}>
                {selectedUser.user.split(' ').map((n:string) => n[0]).join('').slice(0,2)}
              </div>
              <h3 className="text-2xl font-bold text-[#101010] mb-1">{selectedUser.user}</h3>
              ...
              <div className="text-left bg-gray-50 rounded-2xl p-5">
                <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-3 text-center">Interests</h4>
                <div className="flex flex-wrap justify-center gap-2">
                  {(selectedUser.interests || []).length > 0 ? (
                    selectedUser.interests.map((interest: string, i: number) => (
                      <span key={i} className="px-3 py-1.5 rounded-lg bg-white border border-[#101010]/5 text-xs font-medium text-[#101010] shadow-sm">
                        {interest}
                      </span>
                    ))
                  ) : (
