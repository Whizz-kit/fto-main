    ...
    return (
      <div className="max-w-2xl mx-auto space-y-10 pt-8 animate-pulse pb-20">
        ...
        <div className="space-y-5">
           ...
           <div className="h-32 w-full bg-gray-50 rounded-3xl" />
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto space-y-12 pb-24 pt-10">
      ...
      <div className="text-center space-y-6 pt-6 relative">
        ...
        <div className="relative w-36 h-36 mx-auto group perspective-1000">
          <div className="w-full h-full rounded-full bg-white p-2 shadow-xl shadow-gray-200/50 border border-[#101010]/5 transition-transform duration-500 hover:scale-105">
          ...
          {isEditing && (
            <button
               ...
               className="absolute bottom-1 right-1 p-3.5 bg-white rounded-full shadow-lg border border-[#101010]/5 text-[#101010] hover:text-[#066237] transition-all hover:scale-110 active:scale-95 animate-in zoom-in"
               title="Change photo"
            >
        ...
        <div className="space-y-2">
          <h2 className="text-4xl font-bold text-[#101010] tracking-tight">Guest User</h2>
          <p className="text-muted-foreground font-medium text-lg flex items-center justify-center gap-1">
            @guest_explorer
        ...
        {/* Metadata Pills */}
        <div className="flex items-center justify-center gap-4 text-sm text-muted-foreground/80 font-semibold flex-wrap">
          <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full border border-[#101010]/5 shadow-sm cursor-default">
        ...
        {!isEditing && (
          <div className="pt-4">
            <Button 
              ...
              variant="outline"
              className="rounded-full px-8 h-10 border-[#101010]/20 hover:bg-[#101010] hover:text-white transition-all duration-300 font-semibold"
            >
      ...
      {/* Divider */}
      <div className="w-full h-px bg-gradient-to-r from-transparent via-[#101010]/10 to-transparent" />
      ...
      {isEditing && (
        <div className="space-y-4 group animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-6 bg-white rounded-[2rem] border border-[#101010]/10 shadow-inner">
                ...
                <button
                  ...
                  className={`
                    ...
                    ${role === r 
                      ...
                      : "bg-gray-50 text-muted-foreground border-transparent hover:bg-white hover:border-[#066237]/30 hover:text-[#101010]"
                    }
                  `}
                >
                  {r}
        ...
        {isEditing ? (
          <div className="relative animate-in fade-in zoom-in-95 duration-200">
             <textarea
               ...
               className="w-full p-6 rounded-[2rem] bg-white border border-[#101010]/10 focus:outline-none focus:ring-2 focus:ring-[#066237]/20 focus:border-[#066237] min-h-[160px] text-[#101010] leading-relaxed resize-none text-lg shadow-inner transition-all"
               placeholder="Tell us about yourself..."
             />
             <div className="absolute bottom-5 right-6 text-xs text-muted-foreground font-bold pointer-events-none opacity-50">
               {bio.length}/500
             </div>
          </div>
        ) : (
          <div className="p-8 rounded-[2rem] bg-white border border-[#101010]/5 shadow-sm">
            <p className="text-[#101010] leading-relaxed text-lg font-light">
              {bio}
            </p>
          </div>
        )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          {isEditing && (
            <span className="text-xs text-[#066237] font-bold bg-[#066237]/10 px-2.5 py-1 rounded-lg shadow-sm">{selectedInterests.length} selected</span>
          )}
        </div>

        <div className={`flex flex-wrap gap-3 ${isEditing ? 'p-8 bg-white rounded-[2rem] border border-[#101010]/10 shadow-inner animate-in fade-in zoom-in-95 duration-200' : ''}`}>
          {isEditing ? (
                ...
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm transition-all border font-semibold ${
                    isSelected
                      ...
                      : "bg-gray-50 text-muted-foreground border-transparent hover:bg-white hover:border-[#066237]/20 hover:text-[#066237] hover:shadow-sm"
                  }`}
                >
                  {interest}
                </button>
            ...
            selectedInterests.map(interest => (
              <span
                key={interest}
                className="px-5 py-2.5 rounded-xl bg-white text-[#101010] text-sm border border-[#101010]/5 font-semibold shadow-sm cursor-default"
              >
                {interest}
              </span>
            ))
      ...
      {isEditing && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:right-12 md:bottom-10 z-50 animate-in slide-in-from-bottom-10 fade-in duration-300">
          <div className="bg-[#101010] text-white p-2 pl-5 rounded-full shadow-2xl flex items-center gap-5 border border-white/10 ring-4 ring-black/5">
            ...
            <div className="flex items-center gap-2">
              <Button 
                ...
                className="h-10 px-5 rounded-full text-white/70 hover:text-white hover:bg-white/10 font-medium"
              >
                Cancel
              </Button>
              <Button 
                onClick={handleSave} 
                size="sm"
                className="h-10 px-6 rounded-full bg-white text-black hover:bg-gray-100 font-bold hover:scale-105 transition-transform"
              >
                Save Profile
              </Button>
            </div>
